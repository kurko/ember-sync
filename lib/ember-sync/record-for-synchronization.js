...
export default Ember.Object.extend(
  ...
  init: function() {
    ...
    if (!this.get('onlineAdapter')) {
      console.error("onlineAdapter not defined");
    }
  },

  createRemotely: function() {
    var model,
        type = this.get('jobRecord.jobRecordType'),
        serializedRecord = this.propertiesToPersist(),
        adapter = this.get('onlineAdapter').create();

    model = ModelDouble(type, serializedRecord)

    return adapter.createRecord(null, model, model);
  },

  // FIXME This should be removed at the end
  toEmberData: function() {
    ...
    return new Ember.RSVP.Promise(function(resolve, reject) {
      ...
      // FIXME how to deal with relationships?
